...
**세 칸을 한 줄에 두되 캡처는 자르지 않는다.** 박스 비율에 맞춰 잘라 넣었더니 이슈 제목과 보드의 Status 칸이 끊겼다. 지금은 통째로 넣고 박스 쪽을 이미지 비율에 맞춘다 — `build-evidence.mjs`의 `layout()`이 가로 예산을 세 이미지의 비율대로 나눠서, 칸 폭이 서로 다르고 이미지 높이는 셋 다 같아진다.

행을 적게 담은 것도 같은 이유다. 같은 폭에 행을 많이 넣을수록 글자가 작아진다. 브랜치 5줄, 이슈 3건, 보드 카드 1장이면 규칙을 보여주기에 충분하다. 원본은 브라우저를 확대한 상태(1.6 / 2.0 / 1.3배)에서 찍었고, 이슈 목록은 왼쪽 사이드바를 숨겨 제목이 줄바꿈되지 않게 했다.

보드 칸은 표 뷰가 아니라 **칸반 뷰(View 2)** 를 쓴다. 표 뷰는 Title·Assignees·Status가 가로로 늘어서서 1/3 폭에 넣으면 글씨가 뭉갠다. 칸반은 세로라 좁은 칸에 맞고, 카드에 `#99`처럼 그 이슈를 끝낸 PR이 같이 찍힌다.
